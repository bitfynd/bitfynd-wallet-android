...
public class BlockchainServiceImpl extends android.app.Service implements BlockchainService
{
	...
	private void notifyCoinsReceived(@Nullable final Address from, @Nonnull final Coin amount)
	{
		...
		notification.setSound(Uri.parse("android.resource://" + BuildConfig.APPLICATION_ID + "/" + R.raw.coins_received));
			...
			handler.post(new Runnable()
			{
				...
				public void run()
				{
					...
					else
					{
						...
						notification.setSmallIcon(getResources().getIdentifier(
                                "stat_sys_peers_" + (numPeers > 4 ? 4 : numPeers), "drawable", BuildConfig.APPLICATION_ID));
	...
	public void onCreate()
	{
		...
		final String lockName = BuildConfig.APPLICATION_ID + " blockchain sync";
